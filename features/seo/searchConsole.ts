...
// JWT credentials are loaded and OpenSSL-validated by googleAuth.ts before the
// first Google request. The loader supports Render secret files, local JSON,
// and GOOGLE_APPLICATION_CREDENTIALS without exposing private key material.
//
...
import { getSearchConsoleClient } from './googleAuth.js';
...
function googleApiFailure(resp: Response, rawBody: string, jwt: JWT, site: string): GoogleApiFailure {
  ...
  return {
    ...
    rawBody,
    serviceAccountEmail: String((jwt as any).email || 'unknown'),
    site,
  };
}
...
async function fetchQueryReport(jwt: JWT, days: number): Promise<ReportResult> {
  ...
  const token = await getAccessToken(jwt, 'query report');
  if (!token?.token) {
...
async function fetchIndexStatus(jwt: JWT): Promise<IndexStatus> {
  const token = await getAccessToken(jwt, 'index status');
  if (!token?.token) {
...
export async function getSearchConsoleStats(days = 30): Promise<any> {
  const jwt = getSearchConsoleClient();
