...
export const SecurityShieldModule: React.FC<SecurityShieldModuleProps> = ({ currentUser }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const ubqRes = await fetch("/api/admin/unblock-requests");
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/5 p-4 rounded-2xl border border-white/5">
                ...
                {unblockRequests.length > 0 && (
                  <button
                    ...
                    className="px-4 py-2 bg-red-500/10 hover:bg-red-600 text-red-500 hover:text-white rounded-xl text-[11px] font-bold kurdish-text flex items-center gap-2 transition duration-200 shrink-0"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
