...
export default function AdminMovieRoomsPanel({ currentUser, onLogout, onClose }: Props) {
  ...
  const saveRoom = async (event: React.FormEvent) => {
    ...
    if (!isHttpUrl(videoUrl.trim()) && !isHttpUrl(telegramVideoUrl.trim())) {
      ...
      <div className="mx-auto min-h-full max-w-5xl px-5 py-8 md:px-8">
        ...
        {showForm && (
          <form onSubmit={saveRoom} className="mb-8 space-y-4 rounded-3xl border border-white/10 bg-white/5 p-6">
            ...
            <label className="block space-y-2"><span className="text-sm font-bold kurdish-text">لینکی گشتی فیلم (Direct MP4, HLS, Vimeo, Google Drive, Telegram)</span><input type="url" value={telegramVideoUrl} onChange={(e) => setTelegramVideoUrl(e.target.value)} placeholder="https://... (Direct Video URL or Embed Link)" className="w-full rounded-xl border border-white/10 bg-black px-4 py-3 outline-none" /></label>
            ...
            <input inputMode="numeric" value={accessPrice} onChange={(e) => setAccessPrice(e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="نرخی چوونەژوورەوە - بۆ نموونە: 1000 دینار" className="w-full rounded-xl border border-white/10 bg-black px-4 py-3 outline-none" />
            <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-black px-4 py-3 font-bold kurdish-text"><input type="checkbox" checked={showWhatsapp} onChange={(e) => setShowWhatsapp(e.target.checked)} className="h-5 w-5 accent-emerald-500" /><span>پیشاندانی ژمارەی وەتسئەپ لەسەر ژوورەکە</span></label>
